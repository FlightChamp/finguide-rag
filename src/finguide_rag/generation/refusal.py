...
# 1인칭 소유 + 개인 속성.
# "제 신용등급", "내 계좌" 처럼 답변에 개인 정보가 필요한 표현.
RE_OWN = re.compile(
    r"제\s*(신용|개인|거래|계좌|연체|소득|급여|자산|한도|등급|보유|명의)"
    r"|내\s*(신용|계좌|한도|등급|소득|경우|명의)"
)
...
# 1인칭 대명사와 함께 나타날 때만 personalized 로 본다.
...
RE_TIME_VARIANT = re.compile(
    r"현재|오늘|지금|이번\s*(주|달|분기|년)|다음\s*(주|달|분기|년)"
...
def check_question_pattern(question: str) -> RefusalResult | None:
    ...
    # 1인칭 소유 + 개인 속성이면 확실한 personalized
    is_personal = bool(RE_OWN.search(q))

    # 1인칭 대명사만 있으면 무엇을 묻는지 함께 본다.
    # 개인 이력·값을 요구하면 personalized, 일반 규정을 물으면 통과.
    if not is_personal and RE_FIRST_PERSON.search(q):
