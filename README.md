...
병행합니다. 표준 RAG 지표(RAGAs)도 대조 측정했으나 금융 상품 정합성 오류를
잡지 못해 주지표로 채택하지 않았고, 그 근거를 6절에 기록했습니다.
...
무엇을 놓쳤는지는 [6.8절](#68-표준-rag-지표와의-대조-ragas)에 기록했습니다.
...
### 6.8 표준 RAG 지표와의 대조 (RAGAs)

도메인 특화 오류율을 쓰는 것이 편의가 아님을 보이기 위해, RAGAs의 두 지표를
원논문 정의대로 구현해 같은 데이터에 계산하고 사람 라벨과 대조했습니다.
...
**이 두 값은 주지표가 아니라 대조 실험의 산출물입니다.** 측정 대상은 사람
...
바로 이 지점 때문에 **금융 상품 정합성 오류를 잡으려면 도메인 특화 오류율과
사람 검수가 필요**했고, 4.3절의 상품불일치율이 그 결과입니다.

반대 방향도 확인했습니다. Faithfulness가 낮게 준 3건은 **전부 답변의 결함이
아니라 평가 파이프라인의 결함**이었고(조건절 오분해 / 부재 진술 검증 불가 /
진술 추출 시 의미 왜곡), 이는 6.7절에서 자체 평가기에 대해 관측한 현상과
같습니다.
...
- [ ] Context Precision / Recall 측정 (6.8의 미측정 항목)
