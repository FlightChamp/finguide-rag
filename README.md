...
RAG 파이프라인 구축 자체보다, 검색·거절·생성 평가가 실제 금융 업무 리스크를
제대로 측정하는지 검증하는 데 초점을 두었습니다. 그 과정의 기록은
[6절](#6-평가를-어떻게-신뢰하는가)과 [맺음말](#이-프로젝트가-보여주려는-것)에
있습니다.
...
"불가"라고 잘못 안내하게 됩니다. 2026년판 기준으로는 14~19세 구간에
별도 기준이 적용되므로, 구버전 기준의 일괄 "불가" 안내는 오답이 됩니다.
...
| 실무 도입 가능성 | 직접 응답은 리스크가 큼 | Human-in-the-loop 구조로 상대적으로 높음 |
...
이 데모는 **공개 문서만 사용하는 개인 포트폴리오 프로토타입**이며,
하나은행을 비롯한 어떤 금융기관의 공식 서비스도 아닙니다. 제휴·후원·승인
관계가 없습니다. 상품 조건의 정확한 내용은 해당 금융기관의 공식 채널에서
확인하십시오. 배포까지의 판단 과정은 7절에 적었습니다.
...
**질의 처리 — 비용이 낮은 신호부터 소진한다**

```mermaid
flowchart TD
    Q[질의] --> S0{"0단계<br/>질문 패턴"}
    S0 -->|해당| R[답변 보류]
    S0 -->|통과| RET[하이브리드 검색<br/>weighted α=0.5]
    RET --> S1{"1단계<br/>검색 신호"}
    S1 -->|미달| R
    S1 -->|통과| S2{"2단계<br/>LLM 근거 검증"}
    S2 -->|근거 불충분| R
    S2 -->|통과| G[근거 기반 답변 생성]

    style R fill:#FDECEC,stroke:#C0392B,color:#7F1D1D
    style G fill:#E8F5EC,stroke:#0F5132,color:#0F5132
    style S0 fill:#F2F7FA,stroke:#002D56
    style S1 fill:#F2F7FA,stroke:#002D56
    style S2 fill:#FBF8F1,stroke:#8D7150
```

0단계와 1단계는 LLM을 호출하지 않습니다. 2단계(주황)만 호출 비용이 발생하며,
거절 평가셋 53건 중 이 단계까지 도달하는 질의는 22건(42%)입니다.
...
v1.0 시점에는 배포하지 않기로 했습니다. 이유는 세 가지였습니다 —
저작권, API 비용 통제 불가, 인덱스 용량. 셋 다 실제 근거가 있었고,
그래서 판단을 바꾸려면 셋을 따로따로 해소해야 했습니다.

**인덱스 용량** — 가장 먼저 무너진 전제였습니다. 실측해 보니 FAISS 인덱스
4.39MB, 청크 메타 4.39MB, BM25 인덱스 1.95MB로 **합계 10.73MB**였습니다.
GitHub 단일 파일 100MB 한도에 한참 못 미칩니다. "용량 때문에 어렵다"는
판단은 재보지 않고 내린 추정이었습니다. 경량화나 파일 정리 없이
그대로 저장소에 포함했습니다.

**API 비용** — "공개 URL은 호출량을 제한할 수 없다"는 것도 사실이 아니었습니다.
제한할 수 없는 것은 *접속*이고, *비용*은 네 겹으로 막을 수 있습니다.

| 방어선 | 작동 시점 |
|---|---|
| 0단계 규칙 거절 | 즉시 (검색·LLM 호출 0회) |
| 프로젝트 RPM 제한 | 즉시 |
| 모델 허용 목록 2개로 제한 | 즉시 |
| 월 하드 지출 한도 | 한도 도달 시 요청 차단 |

특히 첫 번째가 설계상 이미 존재했다는 점이 결정적이었습니다. 시변 값·
개인정보 조회·범위 외 질문은 검색 이전에 걸러지므로 **비용이 0원**입니다.
거절 로직이 품질 장치이면서 동시에 비용 장치이기도 했던 셈입니다.

**저작권** — 유일하게 기술로 해소되지 않는 항목이었고, 판단으로 정리했습니다.
이 데모가 표시하는 것은 일반에 공개된 상품설명서·약관의 청크 3건이며,
문서명·시행일·청크 ID를 명시한 인용 형태입니다. 문서 전체를 배포하지도,
다운로드를 제공하지도 않습니다. 원본 파일을 저장소에 포함하지 않는 원칙
(11절)은 그대로 유지합니다.

대신 성격 고지를 앱과 README 양쪽에 명시했습니다 — 공개 문서 기반
개인 포트폴리오 프로토타입이며, 어떤 금융기관의 공식 서비스도 아니라는 점.

**배포 후 확인된 것**

로컬과 동일한 인덱스·동일한 모델로 동작하며, 기능 축소나 답변 하드코딩은
없습니다. 다만 무료 티어라 다음 제약이 있습니다.

| 항목 | 내용 |
|---|---|
| 콜드 스타트 | 미접속 상태가 이어지면 재기동에 최대 1분 |
| 임베딩 모델 | 컨테이너 재시작마다 e5-small 재다운로드 |
| 메모리 | 상한 2.7GB. 로컬 실측 1.5GB로 여유가 크지 않음 |
| 코퍼스 시점 | 2026-08 수집분 고정 |

재현이 필요하면 9절 절차로 자신의 코퍼스를 구성할 수 있습니다.
