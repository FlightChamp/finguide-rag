...
def load_metrics() -> dict:
    ...
    if refusal is not None:
        # stages 안에는 단계별 절제 실험 결과가 들어 있다. 앞에서부터
        # 훑으면 "0단계만"(패턴 규칙만 켠 구성)이 먼저 잡혀 최종 성능이
        # 아니라 절제 실험 값이 표시된다. 전체 단계를 켠 구성을 고른다.
        target = refusal
        stages = refusal.get("stages") if isinstance(refusal, dict) else None
        if isinstance(stages, dict) and stages:
            picked = None
            for key, value in stages.items():
                text = str(key)
                if "0+1+2" in text or "LLM" in text.upper():
                    picked = value
                    break
            target = picked if picked is not None else list(stages.values())[-1]

        n_total = None
        if isinstance(refusal, dict):
            n_total = (refusal.get("eval_set") or {}).get("total") \
                if isinstance(refusal.get("eval_set"), dict) else None

        far = deep_find(target, ("false_answer_rate", "far"))
        bal = deep_find(target, ("balanced_accuracy", "balacc"))
        suffix = f" · n={n_total}" if n_total else ""
        if far is not None:
            out.append(("False Answer Rate (FNR)", far,
                        f"거절해야 할 질문에 답한 비율{suffix}"))
        if bal is not None:
            out.append(("Balanced Accuracy", bal,
                        f"거절 판정 · 이진 분류{suffix}"))
...
#: 상품 인식 결과를 최대 몇 개까지 보여줄지. 이보다 많이 잡히면 매칭이
#: 실패한 것으로 보고 아예 표시하지 않는다.
MAX_SHOWN_PRODUCTS = 3

#: 카탈로그 문서명 중 상품명으로 보기 어려운 것을 걸러내는 기준.
#: 제목 추출 단계에서 본문 일부가 딸려 들어간 항목이 섞여 있다.
#: 예: "대출거래약정서 담 당 책 임 자 관 리 자 부 점 장"
MAX_PRODUCT_NAME_CHARS = 30


def looks_like_product(name: str) -> bool:
    """상품명으로 표시하기에 적절한 문자열인가."""
    text = (name or "").strip()
    if not text or len(text) > MAX_PRODUCT_NAME_CHARS:
        return False
    # 한 글자짜리 어절이 여러 개면 본문이 잘못 붙은 경우다.
    parts = text.split()
    if sum(1 for p in parts if len(p) == 1) >= 3:
        return False
    return True
...
def question_products(matcher, names: list[str], question: str) -> list[str]:
    ...
    표시 전용이므로 확신이 없으면 아무것도 내놓지 않는 편이 낫다.
    여러 상품이 한꺼번에 잡히면 어절 겹침으로 잘못 매칭된 것이므로
    전부 버린다.
    """
    ...
    return found if len(found) <= MAX_SHOWN_PRODUCTS else []
...
def enrich_evidences(result: dict, answer_obj: Any) -> dict:
    """근거를 원본 객체에서 다시 읽어 빠진 필드를 채운다.

    Answer.to_dict() 는 근거를 rank / chunk_id / citation / section /
    doc_type / score / text 로만 직렬화한다. effective_date,
    doc_display_name, is_latest 는 빠져 화면에서 N/A 로 나온다.
    파이프라인을 고치는 대신 표시 계층에서 원본 dataclass 를 읽는다.
    """
    raw = getattr(answer_obj, "evidences", None)
    if not raw:
        return result
    ...
    for i, ev in enumerate(raw, start=1):
        try:
            data = asdict(ev) if is_dataclass(ev) and not isinstance(ev, type) \
                else to_result_dict(ev)
        except Exception:
            data = to_result_dict(ev)
        if not isinstance(data, dict) or not data:
            continue
        data.setdefault("rank", i)
        full.append(data)

    if full:
        result = dict(result)
        result["evidences"] = full
    return result
...
def run_pipeline(question: str, offline: bool) -> None:
    ...
    try:
        ...
        st.session_state["result"] = enrich_evidences(
            to_result_dict(answer), answer)
